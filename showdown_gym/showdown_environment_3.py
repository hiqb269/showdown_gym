...
class ShowdownEnvironment(BaseShowdownEnv):
    ...
    def calc_reward(self, battle: AbstractBattle) -> float:
        ...
        if prior_battle is not None:
            ...
            # If my damage is higher than opponent damage, the reward will be negative
            damage_reward = (opp_damage * 0.5) - (my_damage * 0.3)  # Prefer dealing > taking damage
            ...
            ko_reward = (prior_opp_alive - current_opp_alive) * 1.0  # +1.0 per opponent KO
            survival_penalty = (prior_my_alive - current_my_alive) * -1.5  # -1.5 per my KO SHOULD IT BE -1.0??
            reward += ko_reward + survival_penalty

        # 3. Final battle outcome
        if battle.finished:
            if battle.won:
        ...
        # 4. Small turn efficiency penalty to encourage decisive play
        if battle.turn > 30:
            reward -= 0.01 * (battle.turn - 30)  # Very small turn penalty??

        # Clamp final reward to reasonable range for RL training
        reward = np.clip(reward, -3.0, 3.0) #IS THIS NEEDED??
